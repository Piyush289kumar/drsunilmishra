...
$post_id_by_addbar = $_GET['t_id'];

if ($post_id_by_addbar == null) {
    echo ("<script>window.location.href='$hostname/admin/topnews.php'</script>");
...
if (isset($_POST['submit'])) {
    ...
    $sql_update_post = "UPDATE top_post SET tp_id = '{$post_id}' WHERE t_id = '{$post_id_by_addbar}'";
    $result_sql_update_post = mysqli_query($conn, $sql_update_post);
    if ($result_sql_update_post) {
        echo ("<script>window.location.href='$hostname/admin/topnews.php'</script>");
        
    } else {
        echo "<div class='alert alert-danger'>Top News is Not Update !!</div>";
    }
}
 ?>
...
<div id="admin-content">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                    <h1 class="admin-heading">Top News Update</h1>
            </div>
